...
router.get('/artists', paginate(9,18), async (req, res, next) => {
  try {
    ...
    const sql = `SELECT id, slug, title, SUBSTRING(summary,1,300) AS summary, tags, status, image, artist_id, url FROM artists ${where} ORDER BY published_at DESC LIMIT ? OFFSET ?`;
    const [posts] = await db.query(sql, [...params, limit, offset]);
    const [[cnt]] = await db.query(`SELECT COUNT(*) AS cnt FROM artists ${where}`, params);

    res.render('fanart/artists/index', { title: 'Artists', posts, total: cnt.cnt, page, limit, tag: tag || null, q: q || null });
  } catch (e) { next(e); }
});


router.get('/artists/:slug', async (req, res, next) => {
  try {
    const [[post]] = await db.query(`SELECT * FROM artists WHERE slug=?`, [req.params.slug]);
    if (!post) return res.status(404).render('partials/404', { title: 'Not Found' });
    const [comments] = await db.query(`SELECT * FROM artists_comments WHERE post_id=? AND is_approved=1 ORDER BY created_at DESC`, [post.id]);
    res.render('fanart/artists/show', { title: post.title, post, comments });
  } catch (e) { next(e); }
});
...
router.post('/:id/comments', commentRules, async (req, res, next) => {
  try {
    ...
    await db.query(`INSERT INTO artists_comments(post_id, author, body) VALUES(?,?,?)`, [req.params.id, cleanAuthor, cleanBody]);
    res.redirect('back');
  } catch (e) { next(e); }
});
